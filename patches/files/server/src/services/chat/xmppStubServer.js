...
function getUserDataForCharacterId(charId) {
  ...
  const charData =
    ...
    (
      typeof chatRuntime.getSyntheticPresenceMember === "function"
        ? chatRuntime.getSyntheticPresenceMember(charId)
        : null
    );
  if (!charData) {
    return null;
...
function unregisterCharacterSession(session) {
  ...
  // R7b: a socketless (web) corp member never had an XMPP client to unavailable-
  // out, so synthesize its corp departure for the corp room's retail occupants.
  // No-op for a character that still holds a live XMPP client (handled above).
  bridgeSocketlessCorpDeparture(session);
}
...
function handleJoinPresence(client, xml) {
  ...
  if (!descriptor.suppressPresenceBroadcast && charId) {
    ...
    // R7b initial-roster seam: a joining retail client also needs presence for
    // session-derived (socketless) members that hold no XMPP connectedClient —
    // otherwise its opening roster omits the browser/web player entirely. The
    // shared seenCharacterIDs dedup guarantees a character already advertised via
    // a live XMPP client above is never double-listed. Suppressed (delayed-local)
    // rooms never reach this block, matching hidden-until-you-speak.
    for (const memberCharacterID of getSocketlessRoomMemberCharacterIDs(descriptor)) {
      if (
        !memberCharacterID ||
        memberCharacterID === charId ||
        seenCharacterIDs.has(memberCharacterID)
...
// ---------------------------------------------------------------------------
// R7b — session-derived (socketless) presence bridge.
//
// Retail clients build their Local/Corp roster from XMPP MUC presence, which
// only exists for a character holding a live XMPP socket (connectedClients). A
// browser/web session drives chat through chatRuntime/chatHub with NO XMPP
// socket, so retail never saw it. Here xmppStubServer mirrors the socketless
// members chatRuntime already knows about into the MUC it serves retail:
//   * chatRuntime runtime events                                    -> Local
//   * chatRuntime synthetic Corp events                             -> Corp crews
//   * webChatGatewayService.corpChatEmitter                         -> Corp browser
//
// GUARDRAIL (never double-present): a character that already holds a live XMPP
// connectedClient is handled entirely by the existing retail machinery
// (handleJoinPresence / handleGroupMessage / moveSessionToCurrentLocalRoom /
// removeClientFromRooms). Every handler below returns immediately — before any
// side effect — for such a character, so retail<->retail rooms are byte-for-byte
// unchanged. Only socketless characters ever get a synthetic stanza.
//
// Delayed-local suppression is honored implicitly: the runtime only emits a
// delayed-room local-join when a member becomes visible by speaking, and emits
// no presence-bearing local event for a suppressed room otherwise, so mirroring
// the events as they fire preserves the hidden-until-you-speak rule.

function hasConnectedClientForCharacter(characterID) {
  const normalizedCharacterID = Number(characterID || 0);
  if (!normalizedCharacterID) {
    return false;
  }
  for (const client of connectedClients) {
    if (getClientCharacterId(client) === normalizedCharacterID) {
      return true;
    }
  }
  return false;
...
// Connected-player Corp membership remains session-derived and is collapsed to
// one preferred session per character. Synthetic service members stay in
// chatRuntime's separate roster and are merged only at the presentation seam.
function getCorpRoomCharacterSessions(corporationID) {
  ...
  for (const session of sessionRegistry.getSessions()) {
    const sessionCorpID = Number(
      (session && (session.corporationID || session.corpid)) || 0,
    );
    if (sessionCorpID !== corpID) {
      continue;
    }
    const characterID = getCharacterIDForSession(session);
    if (!characterID || preferredByCharacter.has(characterID)) {
      continue;
    }
...
function getCorpRoomOccupants(corporationID) {
  const corpID = Number(corporationID) || 0;
  if (!corpID) {
    return [];
  }
  const members = roomMembers.get(buildConferenceRoomJid(`corp_${corpID}`));
  return members ? [...members] : [];
}

// Socketless (no live XMPP client) member character IDs to seed a joining retail
// client's opening roster with. Local reads the authoritative visible set; Corp
// merges the session registry with chatRuntime's synthetic corporation roster.
// Characters that already hold an XMPP client are excluded — they are advertised
// by the existing occupant-presence broadcast, never twice.
function getSocketlessRoomMemberCharacterIDs(descriptor) {
  if (!descriptor) {
    return [];
  }
  let candidateMembers;
  if (descriptor.kind === "local" && descriptor.roomName) {
    candidateMembers = chatRuntime.getVisibleLocalSessions(descriptor.roomName);
  } else if (descriptor.kind === "corp" && descriptor.roomID) {
    candidateMembers = [
      ...getCorpRoomCharacterSessions(descriptor.roomID),
      ...chatRuntime.getSyntheticCorporationMembers(descriptor.roomID),
    ];
  } else {
    return [];
  }
  ...
  for (const member of candidateMembers) {
    const characterID = getCharacterIDForSession(member);
    if (
      !characterID ||
      seen.has(characterID) ||
      hasConnectedClientForCharacter(characterID)
    ) {
      continue;
    }
...
// Delivery target for a socketless Local event. A normal room advertises to
// every same-system connected pilot (getConnectedClientsForLocalRoom, which
// self-heals a stale room membership exactly like the retail join path). A
// delayed (wormhole/Pochven/nolocal) room must instead target the full XMPP
// occupant set: getConnectedClientsForLocalRoom there is visibility-filtered to
// speakers only, but a silent lurker still needs to receive the message and see
// a speaker appear — matching how handleGroupMessage delivers to roomMembers.
function getLocalRoomBridgeRecipients(roomName, roomJid) {
  if (isDelayedLocalChatRoomName(roomName)) {
    const members = roomMembers.get(roomJid);
    return members ? [...members] : [];
  }
  return getConnectedClientsForLocalRoom(roomName, { roomJid });
}

function bridgeSocketlessLocalJoin(event) {
  try {
    const roomName = normalizeString(event && event.roomName, "").trim();
    const characterID = Number(
      (event && event.summary && event.summary.characterID) || 0,
    );
    if (!roomName || !characterID || hasConnectedClientForCharacter(characterID)) {
      return;
    }
    const roomJid = buildConferenceRoomJid(roomName);
    for (const recipient of getLocalRoomBridgeRecipients(roomName, roomJid)) {
      if (getClientCharacterId(recipient) === characterID) {
        continue;
      }
      sendXml(
        recipient,
        buildRoomPresenceXml(roomJid, characterID, recipient, { available: true }),
      );
    }
  } catch (error) {
    log.debug(`[XMPP] socketless local-join bridge skipped: ${error.message}`);
  }
}

function bridgeSocketlessLocalLeave(event) {
  try {
    const roomName = normalizeString(event && event.roomName, "").trim();
    const characterID = Number(event && event.characterID) || 0;
    if (!roomName || !characterID || hasConnectedClientForCharacter(characterID)) {
      return;
    }
    const roomJid = buildConferenceRoomJid(roomName);
    const recipients = getLocalRoomBridgeRecipients(roomName, roomJid);
    if (recipients.length === 0) {
      return;
    }
    // Mirror removeClientFromRoom for a local occupant: the client-side "leave"
    // admin notice first, then the unavailable presence.
    sendLocalAdminLeave(roomJid, characterID, recipients);
    for (const recipient of recipients) {
      sendXml(
        recipient,
        buildRoomPresenceXml(roomJid, characterID, recipient, { available: false }),
      );
    }
  } catch (error) {
    log.debug(`[XMPP] socketless local-leave bridge skipped: ${error.message}`);
  }
}

function bridgeSocketlessLocalMessage(event) {
  try {
    const roomName = normalizeString(event && event.roomName, "").trim();
    const characterID = Number(event && event.authorCharacterID) || 0;
    const message = normalizeString(event && event.message, "");
    if (
      !roomName ||
      !characterID ||
      message === "" ||
    ...
    deliverRoomMessage(roomJid, characterID, message, recipients, {
      createdAtMs:
        Number(event && event.entry && event.entry.createdAtMs) || Date.now(),
    });
  } catch (error) {
    log.debug(`[XMPP] socketless local-message bridge skipped: ${error.message}`);
  }
}

// Corp presence: retail corp delivery targets the corp_<id> MUC occupant set
// (roomMembers), not the session registry. No local-style admin-leave — corp
// rooms only exchange XMPP presence (removeClientFromRoom sends admin-leave for
// kind==="local" only).
function bridgeSocketlessCorpPresence(corporationID, characterID, available) {
  const corpID = Number(corporationID || 0);
  const normalizedCharacterID = Number(characterID || 0);
  if (
    !corpID ||
    !normalizedCharacterID ||
    hasConnectedClientForCharacter(normalizedCharacterID)
  ) {
    return;
  }
  const roomJid = buildConferenceRoomJid(`corp_${corpID}`);
  for (const recipient of getCorpRoomOccupants(corpID)) {
    if (getClientCharacterId(recipient) === normalizedCharacterID) {
      continue;
    }
    sendXml(
      recipient,
      buildRoomPresenceXml(roomJid, normalizedCharacterID, recipient, {
...
function bridgeSocketlessCorpJoin(event) {
  try {
    bridgeSocketlessCorpPresence(
      event && event.corporationID,
      event && event.characterID,
      true,
    );
  } catch (error) {
    log.debug(`[XMPP] socketless corp-join bridge skipped: ${error.message}`);
  }
}

function bridgeSocketlessCorpLeave(event) {
  try {
    bridgeSocketlessCorpPresence(
      event && event.corporationID,
      event && event.characterID,
    ...
    log.debug(`[XMPP] socketless corp-leave bridge skipped: ${error.message}`);
  }
}

function bridgeSocketlessCorpMessage(event) {
  try {
    const corporationID = Number(event && event.corporationID) || 0;
    const characterID = Number(event && event.authorCharacterID) || 0;
    const message = normalizeString(event && event.message, "");
    if (
      !corporationID ||
    ...
    const roomJid = buildConferenceRoomJid(`corp_${corporationID}`);
    const recipients = getCorpRoomOccupants(corporationID);
    if (recipients.length === 0) {
      return;
    }
    deliverRoomMessage(roomJid, characterID, message, recipients, {
    ...
    log.debug(`[XMPP] socketless corp-message bridge skipped: ${error.message}`);
  }
}

// A socketless (web) corp member has no XMPP client to unavailable-out on
// disconnect, so synthesize its corp departure for the corp room's retail
// occupants. The guard skips any character still holding a live XMPP client —
// that departure is handled by removeClientFromRooms in unregisterCharacterSession.
function bridgeSocketlessCorpDeparture(session) {
  try {
    if (!session) {
      return;
    }
    const characterID = getCharacterIDForSession(session);
    if (!characterID || hasConnectedClientForCharacter(characterID)) {
      return;
    }
    bridgeSocketlessCorpPresence(
      Number((session.corporationID || session.corpid) || 0),
      characterID,
      false,
    );
  } catch (error) {
    log.debug(`[XMPP] socketless corp departure bridge skipped: ${error.message}`);
  }
}

let localChatBridgeSubscribed = false;
function subscribeLocalChatBridge() {
  if (localChatBridgeSubscribed) {
    return;
  }
  localChatBridgeSubscribed = true;
  chatRuntime.on("local-join", bridgeSocketlessLocalJoin);
  chatRuntime.on("local-leave", bridgeSocketlessLocalLeave);
  chatRuntime.on("local-membership-refresh", bridgeSocketlessLocalJoin);
  chatRuntime.on("local-message", bridgeSocketlessLocalMessage);
}

// Reconcile a socketless Corp presence source onto this server. chatRuntime
// supplies synthetic service members directly; webChatGatewayService registers
// its private browser-session emitter at load. Idempotent per emitter.
const corpChatBridgeEmitters = new WeakSet();
function subscribeCorpChatBridge(emitter) {
  if (
    !emitter ||
    typeof emitter.on !== "function" ||
    corpChatBridgeEmitters.has(emitter)
  ) {
    return;
  }
  corpChatBridgeEmitters.add(emitter);
  emitter.on("corp-join", bridgeSocketlessCorpJoin);
  emitter.on("corp-leave", bridgeSocketlessCorpLeave);
  emitter.on("corp-membership-refresh", bridgeSocketlessCorpJoin);
  emitter.on("corp-message", bridgeSocketlessCorpMessage);
}

subscribeLocalChatBridge();
subscribeCorpChatBridge(chatRuntime);
...
module.exports.__test__ = {
  ...
  handleJoinPresence,
  handleReadyIq,
  hasConnectedClientForCharacter,
  normalizeRoomJid,
