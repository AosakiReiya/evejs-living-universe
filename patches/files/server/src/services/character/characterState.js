...
function emitFittingTransactionForSession(session, shipID, changes, options = {}) {
  ...
  const previousSnapshot =
    fittingMoves.length > 0
      ? options.previousSnapshot || peekShipFittingSnapshot(charID, numericShipID)
      : null;
  ...
  if (fittingMoves.length > 0) {
    ...
    for (const change of fittingMoves) {
      ...
      const hasOnlineEffect = getTypeDogmaEffects(
        item.typeID || previous.typeID,
      ).has(EFFECT_ONLINE);
      if (
        nowFitted &&
        !wasFitted &&
      ...
      } else if (wasFitted && !nowFitted && hasOnlineEffect) {
...
function syncFittedModulesForSession(session, shipID = null, options = {}) {
  ...
  for (const moduleItem of syncItems) {
    ...
    const suppressSyntheticModuleInventorySync =
      options.forceSyntheticInventorySync !== true &&
...
function clearCharacterFromSession(session, options = {}) {
  ...
  const wasRetailControlSession = Boolean(
    ...
    (
      (
        ...
        sessionRegistry.isCharacterSessionIndexed(session, oldCharID)
      ) ||
      (
        typeof sessionRegistry.isRegisteredSession === "function" &&
        sessionRegistry.isRegisteredSession(session) &&
        sessionRegistry.resolveSessionCharacterID(session) === Number(oldCharID)
      )
    ),
  );
  ...
  if (wasRetailControlSession && options.controlTransition !== false) {
    ...
    characterControlRuntime.recordRetailSessionEnded(oldCharID, {
      session,
      reason: String(options.controlReason || "retail_character_cleared"),
    });
  }
