...
function resolveCombatActorClass(entity) {
  ...
  if (npcEntityType === ENTITY_TYPE.CONCORD) {
    return ENTITY_TYPE.CONCORD;
  }
  if (npcEntityType === ENTITY_TYPE.NPC) {
...
function isIgnoredInvulnerablePlayer(target) {
  ...
  if (targetClass !== ENTITY_TYPE.PLAYER && targetClass !== ENTITY_TYPE.DRONE) {
    return false;
  }
...
// Resident track R1: living-universe NPCs only admit a player as a combat target when the player
// is actively hostile to the NPC's faction (recent aggression timer or standing below the floor).
// Mirrors the isIgnoredInvulnerablePlayer exemption shape: returns true when the player target
// must be EXCLUDED. Keyed on livingUniverseFlightID — exclusive to living flights (industrial and
// mining crew hulls reuse livingUniverseActorID and must stay on legacy targeting). Non-living
// NPCs and non-player targets are unaffected, and disabling livingHostilityEnabled restores the
// legacy posture entirely.
function isLivingPlayerTargetWithoutHostility(entity, target) {
  if (!entity || !entity.livingUniverseFlightID) {
    return false;
  }
  if (!factionHostilityRuntime.isEnabled()) {
    return false;
  }
  if (resolveCombatActorClass(target) !== ENTITY_TYPE.PLAYER) {
    return false;
  }
  const characterID = getEntityCharacterID(target);
  if (!characterID) {
    return false;
  }
  const factionID = toPositiveInt(entity.warFactionID, 0);
  if (!factionID) {
    return true;
  }
  return !factionHostilityRuntime.isHostile(characterID, factionID);
}
...
function isFriendlyCombatTarget(entity, target) {
  ...
  if (sourceClass === ENTITY_TYPE.CONCORD) {
    return targetClass === ENTITY_TYPE.CONCORD;
  }
  if (sourceClass === ENTITY_TYPE.NPC) {
    return targetClass === ENTITY_TYPE.NPC;
  }
...
function resolveAutoAggroTargetClasses(behaviorProfile) {
  ...
  switch (targetPreference) {
    ...
    case "preferredtargetthennearestnonconcord":
      return [ENTITY_TYPE.PLAYER, ENTITY_TYPE.NPC];
    case "nearesteligible":
    case "preferredtargetthennearesteligible":
      return [ENTITY_TYPE.PLAYER, ENTITY_TYPE.NPC, ENTITY_TYPE.CONCORD];
    case "nearestplayer":
...
function isTargetWithinEntityVisibilityScope(entity, target) {
  return canEntitiesInteractLocally(entity, target);
}
...
function isValidCombatTarget(entity, target, options = {}) {
  ...
  return Boolean(
    ...
    !isLivingPlayerTargetWithoutHostility(entity, target) &&
    (
      !isCapsuleTarget ||
      (
...
function setNpcChaseVelocityMode(entity, mode, policy) {
  ...
  applyNpcChaseMaxVelocityCommand(entity, desiredMaxVelocity);
...
function broadcastNpcChaseVelocityChange(scene, entity, previousState = {}) {
  if (
    scene &&
...
function applyNpcChaseVelocityMode(scene, entity, mode, policy) {
  const previousState = {
    inertia: entity && entity.inertia,
    mass: entity && entity.mass,
    maxVelocity: entity && entity.maxVelocity,
  };
  const changed = setNpcChaseVelocityMode(entity, mode, policy);
  if (changed) {
    broadcastNpcChaseVelocityChange(scene, entity, previousState);
  }
  return changed;
}
...
function isFriendlyNpcAssistanceTarget(entity, target, sourceController = null) {
  if (
    ...
    resolveCombatActorClass(entity) !== ENTITY_TYPE.NPC ||
    resolveCombatActorClass(target) !== ENTITY_TYPE.NPC ||
...
function syncNpcHostileModules(scene, entity, target) {
  ...
  for (const { moduleItem, definition, effectName } of modules) {
    ...
    // Entity-EWAR modules (SDE `entity*Chance` families) roll their activation chance per attempt.
    // A failed roll simply skips this cycle; the next think re-rolls. Real modules (chance
    // undefined) always activate. Roll cadence == think interval, the documented parity shortcut.
    // (Effect strength/range/duration resolve from the rat's own SDE hull attributes, since the
    // synthetic module's typeID is the entity typeID.)
    const entityEwarChance = Number(moduleItem && moduleItem._npcEntityEwarActivationChance);
    if (Number.isFinite(entityEwarChance) && entityEwarChance < 1) {
      if (!(entityEwarChance > 0) || !(Math.random() < entityEwarChance)) {
        continue;
      }
    }
...
function activateNpcSyntheticPropulsion(scene, entity, behaviorProfile) {
  ...
  const effectState = {
    ...
    repeat: null,
    consequenceDelivery: "continuous",
